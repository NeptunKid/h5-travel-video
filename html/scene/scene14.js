;require(['anole', 'zepto'], function(anole, Scene){
	...
	scene.animation = function() {
		...
		this.tl.set(this.map_ctn,{scaleX:5,scaleY:5})
			...
			.to(this.path[1],time_delta,{"stroke-dashoffset":0,ease:Linear.easeNone,delay:time_delta/4})
			.call(function(){$("#scene14 .gate-ctn").remove();})
			.set(this.mark[0],{x:"-100%"})
			.to(this.mark[1],time_delta/3,{opacity:1,y:"-100%"},"-=" + time_delta*5)
			.to(this.mark[2],time_delta/3,{opacity:1,y:"-100%"},"-=" + time_delta*3.5)
			.to(this.mark[3],time_delta/3,{opacity:1,y:"-100%"},"-=" + time_delta*3)
			.to(this.mark[0],time_delta,{opacity:1,x:"0%"})
			.to([this.mark[1],this.mark[2],this.mark[3],],time_delta*2,{y:"-1300%"},"+=1")
			.to([this.mark[0]],time_delta*2,{y:"-150%"},"-="+time_delta*2)
	}
