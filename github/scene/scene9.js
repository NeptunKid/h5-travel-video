;require(['anole', 'zepto', 'TweenLite', 'TimelineLite', 'CSSPlugin', 'EasePack'], function(anole, Scene){
	...
	scene.createDom = function() {
		...
		var genMoney = function(option,cls,prtNode){
			for(var i=0;i<option.length;i++){
				...
				var t=opt.top;
				var n=opt.num;
				var moneyi = $('<div></div>').addClass(cls).css('left',l).css('top',t);
				for(var j=0;j<n;j++){
					$('<div></div>').appendTo(moneyi).addClass('bottom').css('top',(20*j)+'px').css('left','-52px').css('opacity',0.6);
				}
		...
		var genSatis = function(option,cls,prtNode){
			for(var i=0;i<option.length;i++){
				...
				var l=opt.left;
				var t=opt.top;
				var satisi = $('<div></div>').addClass(cls).css('left',l).css('top',t).css('z-index',606);
		...
		// }];
		var money_option=[{
			left:30,top:-450,num:30
		}];
		genMoney(money_option,'flat-money',this.happymoney);
		$('<img src="/resource/dollarpiles1.png"></img>').appendTo(this.happymoney);
		...
		$('<img src="/resource/dollarpiles2.png"></img>').appendTo(this.sadmoney);
		...
		anole.canvas.addClass('strips-9-10');
	...
	scene.animation = function() {
		...
		var flat5=$(this.happymoney.find(".flat-money")[0]);
		...
		this.tl.addLabel('begin')
			...
			.to(this.money, 0.2, {scale:0.3,x:"+=54",y:"+=100"})
			
			.addLabel("happymoney")
			.set(this.happymoney, {/*scale:money_scale,*/ opacity:1})
			.set(flat5, {scale:money_scale, opacity:1})
			...
			.to(this.happymoney.find("img"),0.1,{opacity:1,ease:Power4.easeIn})
			
			.addLabel("happysatis","+=1.6")
			.set(this.sadmoney.find("img"),{opacity:1},"happysatis")
	...
	scene.cleanup = function() { // Called before entering next scene.
		...
		this.container.find(".chinese").remove();
		this.container.find(".money").remove();
	};
