;require(['anole', 'zepto'], function(anole, Scene){
	...
	//	this.key_point = this.map_ctn.find(".point.p1").appendTo(this.container);
		...
		this.marco = $("<div></div>").addClass("marco").appendTo(this.board);
		...
		anole.canvas.addClass('strips-11-15');
	...
	scene.animation = function() {
		...
		this.tl.to(this.map_ctn.find("path")[1],dt,{"stroke-width":3})
				...
				.to([this.path_content],dt,{opacity:1},"-="+dt)
				.to([this.c_ctn,this.svg_ctn],dt,{width:"50%",height:"95%",delay:-dt})
				.to(this.items,dt/2,{})//just delay
				.to(this.items,dt,{opacity:1,left:"25%",top:"2.5%",delay:0,ease:Linear.easeIn})
				.to(this.board,dt/2,{rotation:0,delay:0,ease:Linear.easeOut})
				.to([this.textl,this.count_ctn],dt,{opacity:1,delay:0})
		...
		this.tl.to(this.count,dt*4,{});//just delay
		for (var i in deltas){
		...
		this.tl.to(this.count,total,{num:20,r:30,g:188,b:211,ease:Linear.easeNone,onUpdate:this.update_count.bind(this)},"-="+total)
				...
				.to(this.marco,total*0.2,{x:"-10%",rotationZ:-10,rotationY:180,delay:-total*0.2})
	}
